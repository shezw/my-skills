...
2. 按需使用 [request template](./assets/request-template.md) 和 [response template](./assets/response-template.md) 记录输入与分析。
3. 使用 [task template](./assets/task-template.md) 创建或更新 `todo/task-{YMDHMin}.md`。
4. 使用 [results schema](./assets/results-schema.sql) 初始化 `.agent/tests/results.db`，并先写入一条测试记录。
5. 使用 [test template](./assets/test-template.md) 创建 `tests/{scene}-{YMDHMin}-{target}-{id}.md`。
6. 完成开发与验证；失败时更新 `fails`，成功时写入 `passed_time`。
7. 使用 [log template](./assets/log-template.md) 记录阶段结果，并在结束时检查 git 状态。
8. 默认执行提交，提交信息标题概括任务，正文列出核心改动点，可参考 [commit template](./assets/commit-template.md)。若提交被仓库策略或权限阻断，需要在日志中说明原因。
